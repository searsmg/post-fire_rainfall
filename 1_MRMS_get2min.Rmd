...
setwd("/Users/megansears/Documents/MRMS/SPR/2022")

# Unzip all files
unzip <- list.files(path = '/Volumes/Sears_ARS/spr_2021_jun_jul/batch2', pattern = '.gz',
                  full.names = T)
...
# File names of the rasters in Multi Sensor QPE and QPE only
multi_rasters <- list.files("./MultiSensor_rastersSensor/2021", pattern = ".grib2", full.names = TRUE)
folder2_rasters <- list.files("./QPE/2021", pattern = "grib2", full.names = TRUE)

# Function to perform the division (multi_qpe / qpe) and save raster
perform_division <- function(raster1, raster2, result_folder) {
  
  library(raster)
  
  # Extract the last 21 digits (i.e., datetime) from the file name (datetime)
  ...
  if (last_21_digits1 == last_21_digits2) {
    ...
    r1 <- projectRaster(r1, crs = '+init=EPSG:26913')
    
    r1 <- raster::crop(r1, bbox)
    
    r2 <- raster(raster2) 
    
    r2 <- projectRaster(r2, crs = '+init=EPSG:26913')
...
result_folder <- "./MultiSensor_QPE_divide/2021"  # Path to the folder to store the resulting rasters
...
# Division using parallel processing
...
dir <- './MultiSensor_QPE_divide/2022'
...
# Path to 2 min data
spr_path <- "/Volumes/Sears_ARS/spr_2021_jun_jul/batch2"

# Get the list of GRIB2 files in the folder
files <- list.files(spr_path, pattern = "*.grib2", full.names = TRUE)

# Set the number of cores (8-10 max on this machine)
num_cores <- 7
...
# Read grib2, project raster
process_file <- function(file) {
  
  # Read the GRIB2 file as a raster
  r <- raster(file)
  
  r <- projectRaster(r, crs = '+init=EPSG:26913')
  ...
  # Remove unnecessary objects (or will fill up memory on machine)
  rm(r)
}
...
# Set the folder path
dir <- "/Users/megansears/Documents/MRMS/SPR/2021/processed_tifs/batch2"
...
# Define the folder paths for the result (result of the divide) and precip rate rasters
result_folder <- "/Users/megansears/Documents/MRMS/MultiSensor_QPE_divide/2021"
precip_folder <- "/Users/megansears/Documents/MRMS/SPR/2021/processed_tifs/batch2"
multiplied_folder <- "/Users/megansears/Documents/MRMS/correctedSPR/2021" # where the rasters will go

# List of precip rate raster files
  ...
  for (i in 1:n) {
    if (precip_hour[i] == "23") {
      ...
      if (precip_minute[i] == "00") {
        ...
        # Otherwise, set the result hour to "00"
        result_hour[i] <- "00"
      ...
      # Otherwise, increment the hour as before
...
foreach(precip_file = precip_files, .packages = c("raster")) %dopar% {
  ...
  if (file.exists(precip_file) && file.exists(result_file)) {
    ...
    # Read the result raster
    result_raster <- raster(result_file)
    
    # Multiply the precip rate raster by the result raster
    ...
    # Save the multiplied raster to the output file
...
mult <- raster('/Users/megansears/Documents/MRMS/correctedSPR/test2/multiplied_20220508-230000.tif')
corr <- raster('/Users/megansears/Documents/MRMS/MultiSensor_QPE_divide/2022/result_20220508-230000.tif')
spr <- raster('/Users/megansears/Documents/MRMS/SPR/2022/processed_tifs/PrecipRate_00.00_20220508-230000.tif')
