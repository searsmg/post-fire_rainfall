...
all_2023 <- read_csv('./data/final/mrms/catchments_2023_mrms.csv') %>%
  dplyr::select(-1) %>%
  rename(ID = catchment) %>%
...
all_2021 <- read_csv('./data/final/mrms/catchments_2021_mrms.csv') %>%
  dplyr::select(-1) %>%
  mutate(datetime = force_tz(datetime, tzone = 'MST')) %>%
  filter(!ID == 'MiddleUnburned')

all_2022 <- read_csv('./data/final/mrms/catchments_2022_mrms.csv') %>%
  dplyr::select(-1) %>%
  mutate(datetime = force_tz(datetime, tzone = 'MST')) %>%
  filter(!ID == 'MiddleUnburned')
...
write_csv(all, './data/final/mrms/all_combined.csv')
