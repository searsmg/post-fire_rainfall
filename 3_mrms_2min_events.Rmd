...
all <- bind_rows(all_2021, all_2022) %>%
...
me_stage_test <- me_intens %>%
  ...
  mutate(stage_rise_cm = max(Stage_cm) - first(Stage_cm)) #%>% #,
         #lag2peak_hr = as.numeric(datetime_maxstage - starttime_rain))
  #filter(stage_rise_cm > 2)
  
# Create a list of plots, one for each event
# plots_list <- me_stage_test %>%
#   group_split(event) %>%
#   map(~ ggplot(.x, aes(x = datetime, y = Stage_cm)) +
#           geom_line() +
#           labs(title = paste("Event", unique(.x$event))))
# 
...
#write.csv(me_response_summary, 'me_response_summary.csv')
...
hm_intens <- hm_intens %>%
    mutate(endtime_plus12 = end_time + 8 * 60 * 60,
           starttime_minus1 = start_time - 6 * 60 * 60)
...
response_all <- response_all %>%
  mutate(treatment = if_else(site %in% c('me',
         ...
         area = if_else(site %in% c('me',
                                    ...
                                    'uw'),
                        'Bennett', 'ET'),
         year = if_else(end_time < '2021-12-01 00:00:00',
                        '2021', '2022'),
