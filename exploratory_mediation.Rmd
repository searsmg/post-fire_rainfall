...
plots <- map(cols_to_plot, ~ {
  col_name <- names(response)[.x]
  
  ggplot(response, aes(x = .data[[col_name]])) +
...
paths <- read_csv('./data/final/exploratory_mediation/paths_all_bs1000_enet.csv') %>%
...
ggsave(
  './figures/SR_response/median_indirect_all_sig.png',
       dpi=600,
       width=16,
       height=10)
