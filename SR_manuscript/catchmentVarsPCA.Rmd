...
# adding is pdsi after the fact
pdsi <- read_csv('./data/final_model_inputs/pdsi.csv') %>%
  rename(pdsi_date = Date) %>%
  mutate(pdsi_date = as_date(pdsi_date),
...
# add pdsi to response df
response <- response %>%
  mutate(
    pdsi_match = map2(site, date, function(s, d) {
      pdsi %>%
        filter(site == s, pdsi_date <= d) %>%
        slice_max(pdsi_date, n = 1, with_ties = FALSE) %>%
        dplyr::select(pdsi, pdsi_date)
    })
  ) %>%
  unnest(pdsi_match)
...
# Change category vars to numeric
response_cor <- response %>%
  mutate(sp_cat = ifelse(sp_cat == 'high', 1, 0),
         fire   = ifelse(fire == 'etf', 1, 0))


# Cleaner variable names for display
response_cor_named <- response_cor %>%
  select(-c(stage_rise_cm, lag2peak_halfP, lag2peak_Pstart,
            fire_no, site_number)) %>%
  rename(
    'Snow zone'        = sp_cat,
    'Fire'             = fire,
    ...
    'PDSI'             = pdsi,
    'Year'             = year,
    'Hypso 75'         = terrain_hypso75,
    'Hypso 50'         = terrain_hypso50,
    'Hypso 25'         = terrain_hypso25,
    'Hypso integral'   = terrain_hypsoint,
    'Connectivity'     = terrain_connec,
    'Conn × dNBR'      = terrain_conn_dnbr,
    'Area'             = area_km2,
    'RC circle'        = terrain_rc_circle,
    'Slope > 40'       = terrain_slope40,
    'Slope > 30'       = terrain_slope30,
    'Flow length'      = terrain_flowlength,
    'Valleys/hollows'  = terrain_vall_holl,
    'Recharge'         = terrain_recharge,
    'Glacial'          = terrain_glacial,
    'Elongation'       = terrain_elongation,
    'Bedrock depth'    = soil_brockdepmin,
    'Sand'             = soil_sand,
    'Clay'             = soil_clay,
    'Ksat'             = soil_ksat,
    'Avail. water'     = soil_availwaterstor,
    'dNBR'             = mean_dnbr,
    'NDVI'             = NDVI,
    'Snow persistence' = SP_mean,
    'Mulch'            = mulch_frac
  )

# Get correlations
corr_named <- cor(response_cor_named, use = 'pairwise.complete.obs')

# Plot
ggcorrplot(corr_named,
           type    = 'lower',
           method  = 'square',
           hc.order = TRUE,
           lab     = FALSE,
...
highly_corr_list <- caret::findCorrelation(corr_named, 
                                           cutoff = 0.7, 
                                           verbose = FALSE, 
                                           names = TRUE, 
                                           exact=TRUE) %>%
...
# Filter correlations to look at only > abs(0.7)
as.data.frame(as.table(corr_named)) %>%
  rename(Var1 = Var1, Var2 = Var2, r = Freq) %>%
  filter(abs(r) > 0.7, Var1 != Var2) %>%
  mutate(r = round(r, 2)) %>%
  arrange(desc(abs(r)))
