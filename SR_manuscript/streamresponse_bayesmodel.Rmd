...
p_load(tidyverse, 
       ...
       performance,
       beepr,
       loo)   # if you have conflicted package)
...
summary(pdsiModel)
check_model(pdsiModel)
r2(pdsiModel)

loo_pdsiModel <- loo(pdsiModel, k_threshold = 0.7)
print(loo_pdsiModel) # k estimates are good

saveRDS(pdsiModel,
        "./data/bayes_outputs/pdsi_stageRise_bayes.rds")

pdsiPlot <- as_draws(pdsiModel) %>% 
  data.frame() %>%
  select(2:7) %>%
  melt() %>%
  ggplot(., 
...
pdsiSummary <- as_draws(pdsiModel) %>% 
  data.frame() %>%
  select(2:7) %>%
  melt() %>% 
  group_by(variable) %>%
...
summary(pdsiModel_l2p)
check_model(pdsiModel_l2p)
r2(pdsiModel_l2p)

loo_pdsiModel_l2p <- loo(pdsiModel_l2p, k_threshold = 0.7)
print(loo_pdsiModel_l2p)

saveRDS(pdsiModel_l2p,
        "./data/bayes_outputs/pdsi_l2p_bayes.rds")

pdsiPlot_l2p <- as_draws(pdsiModel_l2p) %>%
  data.frame() %>%
  select(2:7) %>%
  melt() %>%
  ggplot(.,
...
pdsiSummary_l2p <- as_draws(pdsiModel_l2p) %>%
  data.frame() %>%
  select(2:7) %>%
  melt() %>%
  group_by(variable) %>%
