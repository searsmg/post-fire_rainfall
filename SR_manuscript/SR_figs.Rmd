...
srpdsi <- readRDS(here("./data/bayes_outputs/pdsi_stageRise_bayes.rds"))

var_levels <- c("MI60*pdsi",
                "pdsi",
                "MI60")

srpdsi_df <- as_draws(srpdsi) %>% 
  data.frame() %>%
  select(2,3,7) %>%
  melt() %>%
    mutate(variable = case_match(variable,
    "MI60_mmhr.pdsi" ~ "MI60*pdsi",
    "pdsi" ~ "pdsi",
    "MI60_mmhr" ~ "MI60")) %>%
   mutate(variable = factor(variable, levels = var_levels))

srpdsi_summary <- srpdsi_df %>%
  group_by(variable) %>%
  summarize(mean = mean(value),
            median = median(value),
            min = min(value),
...
l2ppdsi <- readRDS(here("./data/bayes_outputs/pdsi_l2p_bayes.rds"))

var_levels <- c("MI60*pdsi",
                "pdsi",
                "MI60")

l2ppdsi_df <- as_draws(l2ppdsi) %>%
  data.frame() %>%
  select(2,3,7) %>%
  melt() %>%
  mutate(variable = case_match(variable,
