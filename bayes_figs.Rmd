...
globalLag <- ggplot(lag_df, 
  ...
  theme(
    ...
    #axis.line.x = element_line(color = "black"),  # add bottom line
    #axis.line.y = element_line(color = "black")   # add left line
  ) +
  labs(x = 'Scaled coefficient',
       y = NULL) +
   geom_text(
    data = lag_summary, 
    aes(
      x = 2,  #max(lag_df$value) * 0.95,  # place near the right edge
      y = as.factor(variable),
      label = percent_positive_label
    ),
    inherit.aes = FALSE,  # don't inherit x/y from the main plot
...
# Add Google or system font
font_add_google('DM Sans')  # example: use Lato font
...
globalPlot <- ggplot(global, 
  ...
  stat_pointinterval(.width = c(.66, .95, 1), 
                     ...
                     color = '#D50032FF') +
  paletteer::scale_fill_paletteer_d("nbapalettes::kings_city") +
  #paletteer::scale_fill_paletteer_d("nord::frost") + #nord::frost
  #scale_y_discrete(limits = rev(var_levels)) +
