...
ggplot(ndvi_summary, aes(
  ...
  geom_errorbar(
    aes(ymin = NDVI_mean - NDVI_sd, ymax = NDVI_mean + NDVI_sd),
    position = position_dodge(width = 0.5),  # same dodge as points
    width = 0.1
  ) +
  paletteer::scale_color_paletteer_d("ggsci::alternating_igv") +
  ...
  theme(
    ...
    strip.background = element_rect(fill = "white", color = NA),
    strip.text = element_text(hjust = 0, color='black', face='bold'),
    text = element_text(color = "black", family = 'sans'),
    axis.text = element_text(color = "black"),
    panel.grid.major.x = element_blank(),
    panel.grid.minor.x = element_blank(),
