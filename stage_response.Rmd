...
# divide the area and channel width
response <- response %>%
...
response <- response %>%
  ...
  mutate(lag2peak = as.numeric((datetime_peak - halfP_datetime)/3600))
...
# subset dataframe
subset <- response %>%
  select(stage_rise_channel, MI60_mmhr, fire, lag_pwd,
         hypso75, mean_dnbr, AVG_CLAY, AVG_KSAT, elongation, 
         aws0150wta, geo_general, NDVI, frac_valley,
         flow_length_m, slope30_frac, SP_mean,
         hypso_integral,
         slope23_frac) %>%
  drop_na()

# RF model
rf_sr_all <- randomForest(stage_rise_channel ~., 
                          mtry=5, ntree=1000, 
                          data=subset)
