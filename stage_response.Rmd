...
numeric_cols <- names(inputs)[2:37]
...
vars <- inputs_tukey %>%
  filter(var %in% c('AVG_CLAY',
                     ...
                     'NDVI_2021.x',
                     'NDVI_2022.x',
                     'NDVI_2023.x')) %>% # vars that are sig diff
...
ggplot(vars, aes(x=fire, 
  ...
  geom_text(aes(label = letter, y = y_text), 
            ...
            size = 6, 
            vjust = 0) + 
  scale_fill_manual(values = c("#9970ABFF", 
                               "#5AAE61FF")) +
  facet_wrap(~var, scales='free_y', ncol=3) +
   scale_y_continuous(expand = expansion(mult = c(0.1, 0.15))) +
...
Rain metrics and PWD

## Corr: stage rise and rain metrics
First, see what rain metrics are most correlated with stage rise
...
# Join dataframes
Join model inputs, PWD, response/rain
...
# add SP category and fire to PWD
pwd <- left_join(pwd, fire, by = 'site')

# make a PWD fig that is faceted by fire and snow zone
ggplot(pwd, aes(x=site,
                y=lag_pwd)) +
  geom_boxplot() +
  facet_wrap(vars(sp_cat,fire), scales='free_x')
