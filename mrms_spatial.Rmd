...
huc <- st_read('./data/GIS/WBDHU2.shp')
...
dem4km <- aggregate(dem, fact = 4000, fun = "mean")

gridmet21 <- rast('./data/mrms_data/bbox_2fires/2021_gridmet_summerp.tif') %>%
  project(., sum_all23)

dem_resam <- resample(dem4km, gridmet21, method = "bilinear")

## cpf 2021
sum21grid_cp <- extract(gridmet21, 
                  cp,
                  xy=T)

dem_sum21grid_cp <- left_join(dem_cp, sum21grid_cp, by = c("x", "y")) %>%
  rename(grid21_total = 6)

ggplot(dem_sum21grid_cp, aes(x=USGS_13_n41w106_20230314,
                         y=grid21_total)) +
  geom_point() +
  ggtitle('Gridmet CPF 2021')

## cpf 2022
gridmet22 <- rast('./data/mrms_data/bbox_2fires/2022_gridmet_summerp.tif') %>%
  project(., sum_all23)

sum22grid_cp <- extract(gridmet22, 
                  cp,
                  xy=T)

dem_sum22grid_cp <- left_join(dem_cp, sum22grid_cp, by = c("x", "y")) %>%
  rename(grid22_total = 6)

ggplot(dem_sum22grid_cp, aes(x=USGS_13_n41w106_20230314,
                         y=grid22_total)) +
  geom_point() +
  ggtitle('Gridmet CPF 2022')

## cpf 2023
gridmet23 <- rast('./data/mrms_data/bbox_2fires/2023_gridmet_summerp.tif') %>%
  project(., sum_all23)

sum23grid_cp <- extract(gridmet23, 
                  cp,
                  xy=T)

dem_sum23grid_cp <- left_join(dem_cp, sum23grid_cp, by = c("x", "y")) %>%
  rename(grid23_total = 6)

ggplot(dem_sum23grid_cp, aes(x=USGS_13_n41w106_20230314,
                         y=grid23_total)) +
  geom_point() +
  ggtitle('Gridmet CPF 2023')


```

## P over MI60 10 mm/hr
...
mi60_p10_23_rast <- rast('/Users/megansears/Documents/MRMS/mrms_data/bbox_2fires/mrms2023_mi60_p10.tif')

# need to resample DEM to match MRMS
dem_resam <- resample(dem1km, mi60_p10_23_rast, method = "bilinear")
...
dem_p10_mi60_cp23 <- left_join(dem_cp, pover10_mi60_cp, by = c("x", "y")) %>%
  mutate(fire = 'cp',
         year = '2023',
         source = 'mrms')

# cp 22
mi60_p10_22_rast <- rast('/Users/megansears/Documents/MRMS/mrms_data/bbox_2fires/mrms2022_mi60_p10.tif')

pover10_mi60_cp <- extract(mi60_p10_22_rast, 
                  cp,
                  xy=T)

dem_p10_mi60_cp22 <- left_join(dem_cp, pover10_mi60_cp, by = c("x", "y")) %>%
  mutate(fire = 'cp',
         year = '2022',
         source = 'mrms')

# cp 21
mi60_p10_21_rast <- rast('/Users/megansears/Documents/MRMS/mrms_data/bbox_2fires/mrms2021_mi60_p10.tif')

pover10_mi60_cp <- extract(mi60_p10_21_rast, 
                  cp,
                  xy=T)

dem_p10_mi60_cp21 <- left_join(dem_cp, pover10_mi60_cp, by = c("x", "y")) %>%
  mutate(fire = 'cp',
         year = '2021',
         source = 'mrms')
...
dem_p10_mi60_et23 <- left_join(dem_et, pover10_mi60_et, by = c("x", "y")) %>%
  mutate(fire = 'et',
         year = '2023',
         source = 'mrms')
...
dem_p10_mi60_et22 <- left_join(dem_et, pover10_mi60_et, by = c("x", "y")) %>%
  mutate(fire = 'et',
         year = '2022',
         source = 'mrms')
...
dem_p10_mi60_et21 <- left_join(dem_et, pover10_mi60_et, by = c("x", "y")) %>%
  mutate(fire = 'et',
         year = '2021',
         source = 'mrms')


dem_p10over <- bind_rows(dem_p10_mi60_cp21,
                         dem_p10_mi60_cp22,
                         dem_p10_mi60_cp23,
                         dem_p10_mi60_et21,
                         dem_p10_mi60_et22,
...
dem_p10over <- dem_p10over %>%
  ...
  mutate(
    elevation_bin = cut(
      elev_m,
      breaks = seq(1700, 4000, by = 150), # Set breaks to 150-meter intervals
      include.lowest = TRUE,
      labels = paste0(
        seq(1700, 3850, by = 150), # Start and end adjusted for 150-meter bins
        "–",
        seq(1850, 4000, by = 150)
      )
    ),
    year = as.factor(year)
  ) %>%
...
# Plot with elevation bins on x-axis and fire as fill
# ggplot(dem_p10over, aes(x = elevation_bin, y = p_over, fill = fire)) +
#   geom_boxplot() +
#   labs(x = 'Elevation Bin (m)', 
#        y = 'P total (mm) over MI60 10 mm/hr', 
#        fill = 'Fire') +
#   theme_bw(base_size = 20) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
#     facet_wrap(~fire+year, scales = 'free')
...
#   ) +
#   scale_fill_manual(
#     values = c("cp" = "#A8B6CC", "et" = "#E57A77"),
#     labels = c("cp" = "CPF", "et" = "ETF")  # Rename legend labels
#   ) +
#   facet_grid(~year)  
# 
# ggplot(dem_p10over, aes(x = elevation_bin, y = p_over, fill = fire)) +
...
#   facet_grid(~year)
# 
# ggsave('./figures/elev_vs_pixel_mrms.png', dpi=600)

ggplot(dem_p10over %>% filter(fire == "cp"),
       aes(x = as.character(median_elev), y = p_over)) +
  geom_violin(scale = 'width', fill = 'steelblue',
              alpha=0.5) +
  stat_summary(
    fun = median,                             # Calculate median
    geom = "point",                           # Plot as points
    size = 2,                                 # Adjust point size
    color = "black",                          # Point color
    position = position_dodge(width = 0.9)    # Align within violins
  ) +
  labs(x = 'Elevation (m)', 
       y = 'P total (mm) over MI60 10 mm/hr', 
       fill = 'Fire') +
...
ggsave('./figures/elev_vs_pixel_mrms_cpf_test.png', dpi=600)

ggplot(dem_p10over %>% filter(fire == "et"),
       aes(x = as.character(median_elev), y = p_over)) +
  geom_violin(scale = 'width', fill = 'steelblue',
              alpha=0.5) +
  stat_summary(
...
ggsave('./figures/elev_vs_pixel_mrms_etf.png', dpi=600)
...
ggplot(dem_p10over,
  ...
  facet_grid(fire ~ year, scales = "free")

ggsave('./figures/elev_vs_pixel_combine.png', dpi=600)
...
gridmet_extract <- gridmet_all %>% 
  pivot_wider(names_from = year, values_from = sum)

gridmet_extract <- st_as_stars(gridmet_all, 
                            dims = c("x", "y", 'year'))

st_crs(gridmet_extract) <- st_crs("EPSG:4326")

cp <- st_transform(cp, "EPSG:4326")

gridmet_extract <- st_extract(gridmet_extract, cp)
...
et <- st_transform(et, "EPSG:4326")

gridmet_extract <- st_extract(gridmet_extract, et)
