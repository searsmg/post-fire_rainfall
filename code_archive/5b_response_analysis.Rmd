...
Need to add all CPF sites in for 2021-2023
...
# all precip
all <- read_csv('/Users/megansears/Documents/Repos/post-fire_rainfall/data/final/mrms/all_combined.csv')

all_intens <- read_csv('/Users/megansears/Documents/Repos/post-fire_rainfall/data/final/mrms/all_mrms_intensities.csv')
...
mw_stage_test <- mw_stage %>%
  mutate(diff = Stage_cm - lag(Stage_cm),
         year = year(datetime),
         month = month(datetime)) %>%
  filter(year == 2022,
         month %in% c(6:9))

look <- ggplot(mw_stage, aes(x=datetime, y=Stage_cm)) + geom_line()
ggplotly(look)
...
ue_intens <- ue_intens %>%
    mutate(endtime_plus12 = end_time + 12 * 60 * 60,
           starttime_minus1 = start_time - 1 * 60 * 60)
...
mpm_precip_stage <- mpm_both %>%
  ...
  ggplot(., aes(x=datetime, y = Stage_cm)) +
  geom_line() + geom_point(shape=1, color='blue') +
  geom_col(aes(y=p_mm*10), color='red') +
  geom_point(aes(y=event))
