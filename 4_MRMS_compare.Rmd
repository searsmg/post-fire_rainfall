...
tb <- as(tb_locations, "Spatial")

mapview(tb)

# bring in larimer co TBs
lar_tb <- read_csv('./data/larimerco_precip.csv') %>%
  dplyr::select(site, lat, long) %>%
  distinct(site, lat, long)

lar_tb_spat <- vect(lar_tb, geom = c("long", "lat"), crs = "EPSG:4326")
lar_tb_sp <- as(lar_tb_spat, 'Spatial')

mapview(lar_tb_sp) + mapview(tb)

rast <- rast('/Users/megansears/Documents/MRMS/bbox_2fires/2min_2021/multiplied_20210501-000000.tif')
mapview(rast) + mapview(lar_tb_sp)
...
# Output dir
output_dir <- "/Users/megansears/Documents/MRMS/bbox_2fires/larimer_tb_pixels_2021/"
...
foreach(fileName = filenames, .packages = c("terra", "lubridate", "dplyr", 'sf', 'tidyverse')) %dopar% {
  ...
  lar_tb <- read_csv('/Users/megansears/Documents/Repos/post-fire_rainfall/data/larimerco_precip.csv') %>%
  dplyr::select(site, lat, long) %>%
  distinct(site, lat, long)

  tb_locations <- vect(lar_tb, geom = c("long", "lat"), crs = "EPSG:4326")
