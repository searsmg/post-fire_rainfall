...
inputs <- read_csv('./data/final_model_inputs/events_inputs.csv') %>%
...
summary <- inputs_combined %>%
  ...
  summarize(median_sr = median(stage_rise_cm),
            median_lag = median(lag2peak_halfP),
            iqr_sr = IQR(stage_rise_cm),
            iqr_lag = IQR(lag2peak_halfP))
...
wilcox.test(lag2peak_halfP ~ sp_cat, data = inputs)
wilcox.test(lag2peak_halfP ~ fire, data = inputs)
...
wilcox.test(lag2peak_halfP ~ year, data = inputs %>%
               filter(fire == 'ETF',
                      sp_cat == 'Intermittent'))
...
ggsave(
  './figures/SR_response/boxplot_sr_l2p.jpg',
       dpi=600,
       width=16,
       height=12)
...
sr_mi60 <- read_csv('./data/final_model_inputs/events_inputs.csv')
...
summary <- sr_mi60 %>%
  group_by(sp_cat, fire, burn_cat) %>%
  summarize(mean_mi60 = mean(MI60_mmhr),
            stage_rise_cm = mean(stage_rise_cm))
...
ggsave(
  './figures/SR_response/mi60_sr.jpg',
       dpi=600,
       width=12,
       height=8)
...
t.test(lag_pwd ~ sp_cat, data = inputs_edit_1)
...
# PWD normal, sp_cat is sig diff: p-value = 6.349e-05, ***
...
ggsave(
  './figures/SR_response/boxplot_vargroup_sig.jpg',
       dpi=600,
       width=12,
       height=12)
