...
bl4_tb <- read_csv('./data/TB_raw/cpf_TB_rain.csv') %>%
  filter(site == 'bl4') %>%
  mutate(datetime = mdy_hm(datetime)) %>%
  arrange(datetime) %>%
  mutate(P_mm = 0.254)
...
bl4_events <- get_events(bl4_tb, bl4_tb$P_mm, bl4_tb$datenumeric,
                          bl4_tb$end, bl4_tb$rain_start)

bl4_events <- get_intensities(bl4_events, bl4_events$event, bl4_tb)

bl4_events <- bl4_events %>%
  mutate(site = 'bl4')

write.csv(bl4_events, './data/final/tb_events/bl4_tb_events.csv')

# read in dry events
dry_events <- read_csv('./data/TB_raw/events_from_qm/dry_precip_events.csv') %>% 
  rename(MI60 = MI60_mmhr,
         P = event_sum_mm,
         starttime = start_time) %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  dplyr::select(MI60, P, starttime) %>%
  mutate(site = 'Dry')

bighorn_events <- read_csv('./data/TB_raw/events_from_qm/bighorn_precip_events.csv') %>% 
  rename(MI60 = MI60_mmhr,
         P = event_sum_mm,
         starttime = start_time) %>%
  mutate(starttime = mdy_hm(starttime)) %>%
...
## combine mub and mont events (read in csvs)
mub_events <- mub_events %>%
  mutate(site = 'MUB')

lpm_events <- lpm_events %>%
  mutate(site = 'LPM')

montgomery_events <-  montgomery_events %>%
  mutate(site = 'Montgomery')

aspen_events <- aspen_events %>%
  mutate(site = 'Aspen')

# lpm doesnt have enough data recorded to compare - lets try usgs gauges
# usgs_events <- read_csv('./data/final/tb_events/usgs_tb_intens.csv') #%>%
#   dplyr::select(starttime=start_time, 
#                 MI60=MI60_mmhr, 
#                 P=event_sum_mm,
#                 site = ID)

events <- bind_rows(mub_events, bighorn_events)
...
library(ggridges)
mi60 <- ggplot(events, aes(x = MI60, y = site)) + 
  geom_density_ridges(
    alpha = 0.7, scale = 0.55,  # Increase scale for taller ridges
    jittered_points = TRUE, position = "points_jitter"
  ) +
  labs(x = "MI60 (mm/hr)", y = element_blank()) + 
...
p <- ggplot(events, aes(x = P, y = site)) + 
  geom_density_ridges(
    alpha = 0.7, scale = 0.55,  # Increase scale for taller ridges
    jittered_points = TRUE, position="points_jitter"
  ) +
  labs(x = "Event rainfall (mm)", y = element_blank()) + 
  theme_bw(base_size = 18) +
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
...
combined_plot <- plot_grid(mi60, p, ncol = 2)
combined_plot

ggsave('./figures/tb_only_eg_option2.png',
       plot = combined_plot,
       width = 10,
       height = 5,
       dpi=600)
...
et_rmse_mi30 <- comp_all %>%
  filter(fire == 'ETF') %>% 
  summarize(rmse = rmse(MI30_mmhr.x,
                        MI30_mmhr.y)) %>%
  mutate(R_spearman = 0.41) # previously calculated in stat_cor

et_pbias <- comp_all %>%
  filter(fire == 'ETF')  %>%
  summarize(pbias = pbias(MI60_mmhr.x, MI60_mmhr.y))

cp_pbias <- comp_all %>%
  filter(fire == 'CPF')  %>%
  summarize(pbias = pbias(MI60_mmhr.x, MI60_mmhr.y))

# grab pbias for QM's paper
select_cpf <- comp_all %>%
    filter(site %in% c('bighorn',
                     'dry',
                     'washout',
                     'montgomery',
                     'michigan',
                     'bl4')) %>%
  select(site, MI60_mmhr.x, MI60_mmhr.y) %>%
  mutate(MI60_mmhr.y = if_else(is.na(MI60_mmhr.y), 0, MI60_mmhr.y)) %>%
  summarize(pbias = pbias(MI60_mmhr.x, MI60_mmhr.y))
...
et1 <- comp_all %>%
  ...
  theme(
    ...
    annotate("text", x = 15, y = 32, label = "R = 0.41, RMSE = 6.5", 
          color = "black", size = 4)
...
ggsave('./figures/pixel_tb_ETmarg_mi30.png',
       plot = et_marg,
       width = 7,
       height = 5,
       dpi=600)
...
all <- bind_rows(comp_all, filter) %>%
  mutate(side = if_else(fire == 'ETF', 'West', 'East'))
...
cp_rmse_mi60 <- all %>%
  ...
  summarize(rmse = rmse(MI60_mmhr.x,
                        MI60_mmhr.y)) %>%
  mutate(R_spearman = 0.50) # previously calculated in stat_cor

cp_rmse_sum <- all %>%
  filter(side == 'East') %>% 
  summarize(rmse = rmse(event_sum_mm.x,
                        event_sum_mm.y)) %>%
  mutate(R_spearman = 0.55) # previously calculated in stat_cor

cp_rmse_mi30 <- all %>%
  filter(side == 'East') %>% 
  summarize(rmse = rmse(MI30_mmhr.x,
                        MI30_mmhr.y)) %>%
  mutate(R_spearman = 0.48) # previously calculated in stat_cor
...
cp1 <- all %>%
  ...
  theme(
    ...
    legend.text = element_text(size = 16),
     legend.title = element_text(size = 16)) +
  annotate("text", x = 80, y = 102, label = "R = 0.48, RMSE = 8.8",
           color = "black", size = 4)
...
ggsave('./figures/pixel_tb_eastmarg_mi30.png',
       plot = cp_marg,
       width = 7,
       height = 5,
       dpi=600)
...
cp1 <- all %>%
  ...
  theme(
    ...
    legend.text = element_text(size = 16),
    legend.title = element_text(size = 16)) +
  annotate("text", x = 75, y = 110, label = "R = 0.55, RMSE = 7.2", 
           color = "black", size = 4)
...
ggsave('./figures/pixel_tb_eastmarg_eventsum.png',
       plot = cp_marg,
       width = 7,
       height = 5,
       dpi=600)
...
Figure 3 in rainfall manuscript

```{r}

# start 1 by 1 since they all have diff formats
aspen <- read_csv('./data/final/tb_events/aspen_tb_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
  dplyr::select(site, MI60)

bighorn <- read_csv('./data/final/tb_events/bighorn_precip_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
...
bl4 <- read_csv('./data/final/tb_events/bl4_tb_events.csv') %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
dadd <- read_csv('./data/final/tb_events/dadd_tb_events.csv') %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
dry <- read_csv('./data/final/tb_events/dry_precip_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
...
hum <- read_csv('./data/final/tb_events/hum_tb_events.csv') %>%
  mutate(year = year(start_time),
...
lpm <- read_csv('./data/final/tb_events/lpm_tb_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
...
me <- read_csv('./data/final/tb_events/me_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
michigan <- read_csv('./data/final/tb_events/michigan_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
mm <- read_csv('./data/final/tb_events/mm_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
  dplyr::select(site, MI60 = MI60_mmhr)

montgomery <- read_csv('./data/final/tb_events/montgomery_tb_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
...
mtcampus <- read_csv('./data/final/tb_events/mtcampus_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
mub <- read_csv('./data/final/tb_events/mub_tb_events.csv') %>%
  mutate(starttime = mdy_hm(starttime)) %>%
  mutate(year = year(starttime),
         month = month(starttime)) %>%
  filter(year %in% c(2021:2023),
...
mw <- read_csv('./data/final/tb_events/mw_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
p1 <- read_csv('./data/final/tb_events/p1_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
ue <- read_csv('./data/final/tb_events/ue_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
uw <- read_csv('./data/final/tb_events/uw_tb_events.csv') %>%
  mutate(year = year(start_time),
         month = month(start_time)) %>%
  filter(year %in% c(2021:2023),
         month %in% c(6:9)) %>%
...
# ran tb_xy from code above
tb_xy <- tb_xy %>%
  rename(elev_m = 2) %>%
  mutate(site = if_else(site == 'hum2',
                        'hum',
                        site)) %>%
  dplyr::select(site, elev_m)

tbMI60 <- left_join(tb_events,
                    tb_xy,
                    by='site') %>%
  mutate(elev_type = if_else(elev_m < 2600,
                             'Low',
                             'High')) %>%
  mutate(fire = if_else(site %in% c('hum',
                                    'lpm',
                                    'mub',
                                    'p1'),
                        'West', 'East'))
...
ggplot(tbMI60, aes(x = elev_type, y = MI60)) + 
  #geom_violin(color = "black", trim = FALSE) +  # violin layer
  geom_boxplot(color='gray23') +               # boxplot inside violin
  facet_wrap(~fire) +
  #scale_y_log10() +
...
summary <- tbMI60 %>%
  group_by(fire, elev_type) %>%
  summarize(mean = mean(MI60))

# library(ggridges)
# 
# tbMI60 <- tbMI60 %>%
#   mutate(fire_elev = paste(fire, elev_type, sep = " - "))
# 
# # Plot with the combined group on the y-axis
# mi60 <- ggplot(tbMI60, aes(x = MI60, y = fire_elev)) + 
#   geom_density_ridges(
#     alpha = 0.7, scale = 0.55,
#     jittered_points = TRUE, position = "points_jitter"
#   ) +
#   labs(
#     x = "MI60 (mm/hr)", 
...
write.csv(tbMI60, './data/final/stage_YN_response/tbHighLow.csv')
