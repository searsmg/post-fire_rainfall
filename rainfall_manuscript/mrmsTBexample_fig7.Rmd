...
ggsave('./figures/mrms_tb_drystage_pres.png', dpi=800, width=12, height = 8)
...
ggsave('./figures/mrms_tb_drystage_pres.png', dpi=800, width=12, height = 6)
...
ggsave('./figures/drystage_pres.png', dpi=800, width=12, height = 8)
...
mrms <- read_csv('./data/final/mrms/mrms_catchments_raw/wmean_exact_catchment_2min_all.csv') %>%
  ...
  mutate(p_mm = if_else(p_mm > 0.1, p_mm, 0)) %>% # mrms precision is 0.1
  filter(datetime > ymd_hms('2021-07-20 15:40:00', tz ='MST'),
         datetime < ymd_hms('2021-07-20 17:30:00', tz='MST'))

mrms <- mrms %>%
  mutate(type = 'MRMS')

ggplot(mrms, aes(datetime,
                 p_mm)) +
...
ggsave('./figures/precipExample.png', dpi=800,
       width=12,
       height=8)


mrmsSum <- mrms %>%
  mutate(cumsum = cumsum(p_mm))

ggplot(mrmsSum, aes(datetime,
                 cumsum)) +
...
ggsave('./figures/precipSum.png', dpi=800,
       width=12,
       height=8)
...
# PWD example fig for defense

```{r}

pwd <- rast('/Users/megansears/Downloads/climateEngine_download.wb.tif')
plot(pwd)

shp <- vect('/Users/megansears/Library/CloudStorage/OneDrive-Colostate/PhD/post-fire_rain_response/data/GIS/catchments_all/catchments_all_lidar.shp') %>%
  tidyterra::filter(site == 'UE') %>%
  project(., crs(pwd))
