...
download_prism <- function(url, outdir) {
  ...
  # File paths
  fname_base <- paste(var, region, res, fname_date, sep = "_")
  zipfile <- file.path(outdir, paste0(fname_base, ".zip"))
  unzipdir <- file.path(outdir, paste0(fname_base, "_unzipped"))
  
  # Download ZIP
  resp <- GET(url, write_disk(zipfile, overwrite = TRUE), progress())
...
# Example URLs
urls <- c(
  "https://services.nacse.org/prism/data/get/us/800m/ppt/20210601",
  "https://services.nacse.org/prism/data/get/us/800m/ppt/20210602"
)

# Download and read

# Define date range as a tibble
dates_tbl <- tibble(
...
# Download and read rasters
outdir <- "~/Downloads/prism_800m"
rasters <- dates_tbl %>%
  pull(url) %>%
  map(~ download_prism(.x, outdir))
