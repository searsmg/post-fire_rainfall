...
huc <- st_read('/Users/megansears/Library/CloudStorage/OneDrive-Colostate/PhD/post-fire_rain_response/data/GIS/WBDHU8.shp') %>%
...
write_sf(huc8,
            '/Users/megansears/Library/CloudStorage/OneDrive-Colostate/PhD/post-fire_rain_response/data/GIS/huc8line.shp')
...
########### PLOT JUST FOR STUDY AREA

rast<- rast('/Users/megansears/Documents/MRMS/mrms_data/bbox_2fires/summertotal_2021_fil01.tif')
...
ggsave('./figures/studyarea_zoomedout.png', dpi=800, width=20, height = 15)
...
legend_only <- get_legend(ptot_legend)
...
legend_only
ggsave('./figures/ptotal_legend2.png',plot=legend_only, dpi=600,
       width=15, height = 20)
...
PRISM 800 JJAS precip data was pulled using the pullPRISM800 script
...
# Get raster extent
x_min <- min(prism_all$x)
x_max <- max(prism_all$x)
y_min <- min(prism_all$y)
y_max <- max(prism_all$y)

prism_all_plot <- ggplot() +
  geom_raster(data = prism_all, 
              aes(x = x, y = y, fill = sum)) +
  scale_fill_viridis_c(name = "P total (mm)", 
                       option = "turbo",
                       direction = 1,
...
# save fig
ggsave('./figures/ptot_prism_plot.svg', dpi=600, width=15, height = 20)
...
sd_right21<- sd(as.vector(grid_all21_right[[1]]), na.rm = TRUE)
sd_right22 <- sd(as.vector(grid_all22_right[[1]]), na.rm = TRUE)
sd_right23 <- sd(as.vector(grid_all23_right[[1]]), na.rm = TRUE)


sd_left21<- sd(as.vector(grid_all21_left[[1]]), na.rm = TRUE)
sd_left22 <- sd(as.vector(grid_all22_left[[1]]), na.rm = TRUE)
sd_left23 <- sd(as.vector(grid_all23_left[[1]]), na.rm = TRUE)


msd_right21<- sd(as.vector(sum_all21_right[[1]]), na.rm = TRUE)
msd_right22 <- sd(as.vector(sum_all22_right[[1]]), na.rm = TRUE)
msd_right23 <- sd(as.vector(sum_all23_right[[1]]), na.rm = TRUE)

msd_left21<- sd(as.vector(sum_all21_left[[1]]), na.rm = TRUE)
msd_left22 <- sd(as.vector(sum_all22_left[[1]]), na.rm = TRUE)
msd_left23 <- sd(as.vector(sum_all23_left[[1]]), na.rm = TRUE)
