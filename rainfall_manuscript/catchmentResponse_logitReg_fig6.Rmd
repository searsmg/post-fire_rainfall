...
burn <- read_csv('./data/final_model_inputs/inputs1.csv') %>%
  dplyr::select(site, NDVI_2021, NDVI_2022, NDVI_2023) %>%
  mutate(site = tolower(site))
...
etf_tb <- left_join(etf_tb,
  ...
  mutate(NDVI = case_when(
    year(date) == 2021 ~ NDVI_2021,
    year(date) == 2022 ~ NDVI_2022,
    year(date) == 2023 ~ NDVI_2023
  )) %>%
  dplyr::select(-NDVI_2021, -NDVI_2022, -NDVI_2023)
...
pwd_daymet <- read_csv('./data/final_model_inputs/pwd_daymet.csv') %>%
  select(site,
         date = Date,
         lag_pwd)
...
# 
# 
# fit_kappa <- function(df, metric) {
#   conv <- TRUE
#   mod <- withCallingHandlers(
#     glm(reformulate(c(metric, "lag_pwd", "NDVI"), response = "flow"),
#         data = df, family = 'binomial'),
#     warning = function(w) {
#       if (grepl("did not converge", conditionMessage(w))) conv <<- FALSE
#       invokeRestart("muffleWarning")
#     }
#   )
#   pred_prob <- predict(mod, type = 'response')
#   preds <- factor(if_else(pred_prob < 0.5, 0, 1), levels = c(0, 1))
#   tibble(
#     kappa_value = unname(confusionMatrix(preds, factor(df$flow, levels = c(0, 1)))$overall["Kappa"]),
#     kge_value = KGE(sim = pred_prob, obs = df$flow),
#     converged = conv
#   )
# }
# 
# et_tb_kappa <- etf_tb %>%
#   #group_by(site) %>%
#   reframe(fit_kappa(pick(everything()), metric)) %>%
#   mutate(fire = 'etf',
#          source = 'tb')
# 
# et_tb_kappa
...
# join pwd to cpf_tb
cpf_mrms <- left_join(cpf_mrms,
                    pwd_daymet, # CHECK THIS BEFORE - TESTING WITH DAYMET
                    by = c('site', 'date'))

# cpf_mrms_kappa <- cpf_mrms %>%
#   drop_na(NDVI) %>% 
#   group_by(site) %>%
#   reframe(fit_kappa(pick(everything()), metric)) %>%
#   mutate(fire = 'cpf',
...
# write.csv(kappa, './data/final/stage_YN_response/kappa_KGE_eventsum_wmean_PWDDAYMET_updatedJun2026.csv')
# 
# kappaSummary
...
etf_tb <- etf_tb %>%
  select(flow, MI30_mmhr,
         MI60_mmhr, event_sum_mm,
         date, site, source, fire, NDVI,
         lag_pwd)
...
# this is bad naming but it mean NO UNBURN INCLUDED
full_burn <- left_join(full, burn, by = 'site') %>%
  mutate(burn_cat = if_else(mean_dnbr > 270, 'burn', 'unburned')) %>%
  filter(burn_cat == 'burn')

#write.csv(full_burn, './data/final/stage_YN_response/full_burn_formodel.csv')
write.csv(full_burn, './data/final/stage_YN_response/full_burn_formodel_daymetpwd.csv')

################################################################################

full_burn <- read_csv('./data/final/stage_YN_response/full_burn_formodel_daymetpwd.csv')

metric <- 'MI60_mmhr'

# fit pooled models (keep mod for site-level predictions)
pooled_models <- full_burn %>%
  group_by(fire, source) %>%
  summarise(
    mod = list(glm(reformulate(c(metric, "lag_pwd", "NDVI"), response = "flow"),
                   data = pick(everything()), family = 'binomial')),
    .groups = 'drop'
  )

# pooled-level kappa and KGE
pooled_performance <- pooled_models %>%
  mutate(
    kappa_value = map_dbl(mod, function(m) {
      pred_prob <- predict(m, type = 'response')
      preds <- factor(if_else(pred_prob < 0.5, 0, 1), levels = c(0, 1))
      unname(confusionMatrix(preds, factor(m$y, levels = c(0, 1)))$overall["Kappa"])
    }),
    kge_value = map_dbl(mod, function(m) {
      KGE(sim = predict(m, type = 'response'), obs = m$y)
    })
  ) %>%
  select(-mod)
...
# site-level kappa and KGE using pooled model
site_kappa <- full_burn %>%
  group_by(fire, source, site) %>%
  nest() %>%
  left_join(pooled_models, by = c('fire', 'source')) %>%
  mutate(
    kappa_value = map2_dbl(data, mod, function(site_df, m) {
      pred_prob <- predict(m, newdata = site_df, type = 'response')
      preds <- factor(if_else(pred_prob < 0.5, 0, 1), levels = c(0, 1))
      unname(confusionMatrix(preds, factor(site_df$flow, levels = c(0, 1)))$overall["Kappa"])
    }),
    kge_value = map2_dbl(data, mod, function(site_df, m) {
      KGE(sim = predict(m, newdata = site_df, type = 'response'), obs = site_df$flow)
    })
  ) %>%
  select(-data, -mod)

# site_kappa_summary <- site_kappa %>% 
#   group_by(fire, source) %>% 
#   summarize(mean = mean(kappa_value),
#             .groups = 'drop')
# 
# site_kge_summary <- site_kappa %>% 
#   group_by(fire, source) %>% 
#   summarize(mean = mean(kge_value),
#             .groups = 'drop')

print(site_kappa, n=50)

write_csv(site_kappa, './data/final/stage_YN_response/mi60_model_site_Jun2026_daymetpwd.csv')
