...
aspen_tb <- read_csv('./data/TB_raw/cpf_TB_rain.csv') %>%
  filter(site == 'aspen') %>%
  mutate(datetime = mdy_hm(datetime)) %>%
  arrange(datetime) %>%
  mutate(P_mm = 0.254)

aspen_tb <- get_setup(aspen_tb, aspen_tb$datetime)

aspen_events <- get_events(aspen_tb, aspen_tb$P_mm, aspen_tb$datenumeric,
                          aspen_tb$end, aspen_tb$rain_start)

aspen_events <- get_intensities(aspen_events, aspen_events$event, aspen_tb)

write.csv(aspen_events, './data/final/tb_events/aspen_tb_events.csv')
...
cp1 <- all %>%
  ...
  arrange(desc(rqi)) %>%
  ggplot(., 
         aes(y = MI60_mmhr.y, x = MI60_mmhr.x, color = rqi_bucket)) +
  geom_point(alpha = 0.6, size = 4, stroke = 1) +
  scale_color_manual(values = color_palette) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
   stat_cor(method = "spearman", 
           ...
           hjust = 0.5,
           vjust = -1) +
  labs(x = 'MRMS MI60 (mm/hr)', y = 'TB MI60 (mm/hr)', color = 'RQI') +
  theme_bw(base_size = 18) +
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
...
ggsave('./figures/pixel_tb_eastmarg_mi60.png',
       plot = cp_marg,
       width = 7,
       height = 5,
       dpi=600)
